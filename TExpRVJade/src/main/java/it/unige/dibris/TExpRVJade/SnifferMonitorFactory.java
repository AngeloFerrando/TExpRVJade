...
import it.dibris.unige.TExpSWIPrologConnector.exceptions.NoMinimalMonitoringSafePartitionFoundException;
import it.dibris.unige.TExpSWIPrologConnector.exceptions.NotMonitoringSafePartitionException;
import it.dibris.unige.TExpSWIPrologConnector.texp.TraceExpression;
import it.unige.dibris.TExpRVJade.exceptions.JADEContainerInitializationException;
import it.unige.dibris.TExpRVJade.monitor.Sniffer;
...
public class SnifferMonitorFactory {
	...
	public static Monitor createCentralizedMonitor(TraceExpression tExp, List<AgentController> agents) throws StaleProxyException{
		...
		String agentsStr = "[";
		for(String agent : tExp.involvedRoles()){
			agentsStr += agent + ",";
		}
		agentsStr = agentsStr.substring(0, agentsStr.length() - 1) + "]";
